    ...
    if request.method == 'GET':
        ...
        try:
            for i in request.form.get('public').split(' '):
                ...
                datestr = enddate
                AFCDATA[i] = {'encrypt':None,'maximum':None,'delafter':datestr,'maxviews':None,'watermark':None}
            for i in request.form.get('internal').split(' '):
                i = i.strip(',')
                print('internal: ',i)
                enddate = datetime.now() + timedelta(days=7) #7 days
                datestr = enddate
                AFCDATA[i] = {'encrypt':None,'maximum':3600,'delafter':datestr,'maxviews':None,'watermark':'Internal Data. Do not share.'}
            for i in request.form.get('private').split(' '):
                i = i.strip(',')
                print('private: ',i)
                enddate = datetime.now() + timedelta(days=1) #24 hours
                datestr = enddate
                AFCDATA[i] = {'encrypt':None,'maximum':120,'delafter':datestr,'maxviews':None,'watermark':'Restricted Data. Highly Sensitive.'}
