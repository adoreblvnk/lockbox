...
from flask import (abort, flash, jsonify, redirect, render_template,
                   request, send_file, session)  # requires this to access session attrs
import hashlib
...
class AESCipher(object):
    ...
    def newencrypt(self, raw):
        ...
        encryptedOutput = 'AES'+(base64.b64encode(iv)).decode('utf-8')+'||'+(
            base64.b64encode(cipher.encrypt(raw.encode()))).decode('utf-8')
    ...
    def encrypt(self, raw):
        ...
        cipher = AES.new(self.key, AES.MODE_CBC, iv)
        encryptedOutput = '' + \
            (base64.b64encode(iv + cipher.encrypt(raw.encode()))).decode('utf-8')
        return encryptedOutput
...
const myTimeout = setTimeout(DoDecrypt, 500);
    ...
    try:
        ...
        if isinstance(attributeDict['maximum'], int):
            newmodal = SESSIONMODAL.replace(
                "&&&time&&&", str(attributeDict['maximum']))
            output += newmodal
...
def HandleWatermark(filename, path, initial):
    ...
    watermark_text(
        str(path), "./templates/Mark_AFC_templates/watermarktemp.png", text=watermarkText)
...
def AFCadmin(filename):
    ...
    if isOwner(filename, session['username']) == False:
        abort(404)  # Mark
    if request.method == 'GET':
        ...
        workdict = {'encrypt': None, 'maximum': None,
                    'delafter': None, 'maxviews': None, 'watermark': None}
        ...
        try:
            ...
            if request.form.get('delafter?') == '1':
                delafter = datetime.strptime(
                    request.form.get('delafter'), '%Y-%m-%dT%H:%M')
...
def bigadmin():
    ...
    if request.method == 'GET':
        ...
        print(request.form.keys())
        try:
            for i in request.form.get('public').split(' '):
                i = i.strip(',')
                print('public: ', i)
                enddate = datetime.now() + timedelta(days=365)  # 1 year
                datestr = enddate
                AFCDATA[i] = {'encrypt': None, 'maximum': None,
                              'delafter': datestr, 'maxviews': None, 'watermark': None}
            for i in request.form.get('internal').split(' '):
                i = i.strip(',')
                print('internal: ', i)
                enddate = datetime.now() + timedelta(days=7)  # 7 days
                datestr = enddate
                AFCDATA[i] = {'encrypt': None, 'maximum': 3600, 'delafter': datestr,
                              'maxviews': None, 'watermark': 'Internal Data. Do not share.'}
            for i in request.form.get('private').split(' '):
                i = i.strip(',')
                print('private: ', i)
                enddate = datetime.now() + timedelta(days=1)  # 24 hours
                datestr = enddate
                AFCDATA[i] = {'encrypt': None, 'maximum': 120, 'delafter': datestr,
                              'maxviews': None, 'watermark': 'Restricted Data. Highly Sensitive.'}
            flash('Bulk edits to AFC completed', 'success')
            return redirect("/files")
        except:
            return redirect("/bigadmin")
...
def LockBoard(filename):
    ...
    if request.method == 'GET':
        ...
        for line in Lines:
            ...
            if array[0] == filename:
                logs.insert(0, array)
                if array[3] not in countryValues:
                    countryValues[array[3]] = 1
                else:
                    countryValues[array[3]] = countryValues[array[3]] + 1
